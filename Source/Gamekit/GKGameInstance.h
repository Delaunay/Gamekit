 ...
 * Once you make a blueprint subclass of your native subclass you will want to set it to be the default in project
 * settings
 */
UCLASS()
class GAMEKIT_API UGKGameInstanceBase: public UGameInstance
{
    ...
    /** Synchronously loads a save game. If it fails, it will create a new one for you. Returns true if it loaded, false
     * if it created one */
    UFUNCTION(BlueprintCallable, Category = Save)
    bool LoadOrCreateSaveGame();

    /** Handle the final setup required after loading a USaveGame object using AsyncLoadGameFromSlot. Returns true if it
     * loaded, false if it created one */
    UFUNCTION(BlueprintCallable, Category = Save)
    bool HandleSaveGameLoaded(USaveGame *SaveGameObject);

    /** Gets the save game slot and user index used for inventory saving, ready to pass to GameplayStatics save
     * functions */
    UFUNCTION(BlueprintCallable, Category = Save)
    ...
    /** Resets the current save game to it's default. This will erase player data! This won't save to disk until the
     * next WriteSaveGame */
    UFUNCTION(BlueprintCallable, Category = Save)
    void ResetSaveGame();
