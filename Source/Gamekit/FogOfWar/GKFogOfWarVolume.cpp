...
void  AGKFogOfWarVolume::SetFogOfWarMaterialParameters(FName name, UMaterialInstanceDynamic* Material, bool CreateRenderTarget) {
    if (Material == nullptr) {
    ...
    if (UseFoWDecalRendering && Faction.IsValid()) {
        ...
        if (DecalMaterialInstance == nullptr) {
            DecalMaterialInstance = UKismetMaterialLibrary::CreateDynamicMaterialInstance(
                GetWorld(),
                DecalComponent->GetDecalMaterial(),
                NAME_None,
                EMIDCreationFlags::None);
...
UCanvasRenderTarget2D* AGKFogOfWarVolume::GetFactionRenderTarget(FName name, bool CreateRenderTarget) {
    ...
    if (renderResult != nullptr) {
        ...
        UE_LOG(LogGamekit, Log, TEXT("Creating a new render target for %s (%.2f x %.2f)"),
            *name.ToString(),
            TextureSize.X,
            TextureSize.Y
        );
...
UMaterialParameterCollection* AGKFogOfWarVolume::GetMaterialParameterCollection() {
    return FogMaterialParameters;
}
...
void AGKFogOfWarVolume::SetTextureSize(FVector2D size) {
    ...
    UKismetMaterialLibrary::SetVectorParameterValue(
        ...
        MaterialParameters,
        "TextureSize",
        FLinearColor(size.X, size.Y, 1.f, 1.f)
    );
}
...
void AGKFogOfWarVolume::DrawUnobstructedLineOfSight(UGKFogOfWarComponent* c) {
    ...
    Canvas->K2_DrawMaterial(
        Cast<UMaterialInterface>(material),
        Start,
        NewRadius * 2.f,
        FVector2D(0.f, 0.f),
