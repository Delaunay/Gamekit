...
void FUpscalingDispatcher::BeginRendering()
{
    ...
    if (RendererModule)
    {
        OnPostResolvedSceneColorHandle = RendererModule->GetResolvedSceneColorCallbacks().AddRaw(
                this, &FUpscalingDispatcher::Execute_RenderThread);
    }
}
...
{
    auto ComputeShaderOutputDesc =
            FPooledRenderTargetDesc::Create2DDesc(CachedParams.OriginalSize * CachedParams.Multiplier,
    ...
    GRenderTargetPool.FindFreeElement(
            RHICmdList, ComputeShaderOutputDesc, ComputeShaderOutput, TEXT("FUpscalingShader_Output_RenderTarget"));

    auto ComputeShaderInputDesc =
            FPooledRenderTargetDesc::Create2DDesc(CachedParams.OriginalSize,
    ...
    GRenderTargetPool.FindFreeElement(
            RHICmdList, ComputeShaderInputDesc, ComputeShaderInput, TEXT("FUpscalingShader_Input_RenderTarget"));
}
...
void FUpscalingDispatcher::CopyInputTextureToInputTarget(FRHICommandListImmediate &RHICmdList)
{
    ...
    RHICmdList.Transition(
            FRHITransitionInfo(GetTextureRHI(CachedParams.OriginalTexture), ERHIAccess::Unknown, ERHIAccess::CopySrc));

    // InputTarget is about to be copied to
    RHICmdList.Transition(FRHITransitionInfo(ComputeShaderInput->GetRenderTargetItem().ShaderResourceTexture,
                                             ERHIAccess::UAVCompute,
                                             ERHIAccess::CopyDest));

    // Copy the entire target
    RHICmdList.CopyTexture(GetTextureRHI(CachedParams.OriginalTexture),
                           ComputeShaderInput->GetRenderTargetItem().ShaderResourceTexture,
                           FRHICopyTextureInfo());

    // Set input target back to Compute
    RHICmdList.Transition(FRHITransitionInfo(
            ComputeShaderInput->GetRenderTargetItem().UAV, ERHIAccess::CopyDest, ERHIAccess::UAVCompute));
}
...
void FUpscalingDispatcher::CopyOutputTargetToOutputTexture(FRHICommandListImmediate &RHICmdList)
{
    ...
    RHICmdList.Transition(FRHITransitionInfo(ComputeShaderOutput->GetRenderTargetItem().ShaderResourceTexture,
                                             ERHIAccess::UAVCompute,
                                             ERHIAccess::CopySrc));

    // OutputTexture is about to be copied to
    RHICmdList.Transition(
            FRHITransitionInfo(GetTextureRHI(CachedParams.UpscaledTexture), ERHIAccess::Unknown, ERHIAccess::CopyDest));

    RHICmdList.CopyTexture(ComputeShaderOutput->GetRenderTargetItem().ShaderResourceTexture,
                                                ...
                                                class FSceneRenderTargets &SceneContext)
{
    ...
    FComputeShaderUtils::Dispatch(
            ...
            ComputeShader,
            PassParameters,
            FIntVector(FMath::DivideAndRoundUp(CachedParams.OriginalSize.X, NUM_THREADS_PER_GROUP_DIMENSION),
