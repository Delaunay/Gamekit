...
#define GENERATE_LOG_FUN(Verbosity)                                                                      \
    ...
    void Log_##Verbosity(const char *    Filename,                                                       \
            ...
            TRACE_LOG_MESSAGE(LCategoryName, Verbosity, LFormat, UE_LOG_Args...) UE_LOG_EXPAND_IS_FATAL( \
                    Verbosity,                                                                           \
                    {                                                                                    \
                        FMsg::Logf_Internal(UE_LOG_SOURCE_FILE(Filename),                                \
                                            Line,                                                        \
...
#define GK_ERROR(Format, ...)   GAMEKIT_LOG(LogGamekit, Error, Format, __VA_ARGS__)
