...
AGKHexGrid::AGKHexGrid()
{
    ...
    static ConstructorHelpers::FObjectFinder<UMaterial> DefaultHexMaterial(
            TEXT("Material'/Gamekit/Tiles/TilePlaceholderMat.TilePlaceholderMat'"));
    static ConstructorHelpers::FObjectFinder<UMaterial> DefaultHexMaterialAlt(
            TEXT("Material'/Gamekit/Tiles/TilePlaceholderMat2.TilePlaceholderMat2'"));
...
void AGKHexGrid::LoadCircleMap(int radius)
{
    for (auto i = 0; i < radius; i++)
    {
        ...
        {
            auto points = {FIntPoint(i, j), FIntPoint(-i, j), FIntPoint(i, -j), FIntPoint(-i, -j)};
...
bool AGKHexGrid::ContainsTile(FIntVector w) const { return GridMap.Contains(w); }

UStaticMeshComponent *AGKHexGrid::GetTile(FIntVector w) { return GridMap.FindRef(w); }
...
void AGKHexGrid::BeginPlay() { Super::BeginPlay(); }

// Called every frame
void AGKHexGrid::Tick(float DeltaTime) { Super::Tick(DeltaTime); }
