...
void UGKSharedTexture::BeginPlay()
{
	...
	CPUTexture = RenderTarget->ConstructTexture2D(
		this,			// UObject * Outer,
...
void UGKSharedTexture::AddPoint(FIntPoint Index, FLinearColor Value)
{
	...
	UKismetRenderingLibrary::BeginDrawCanvasToRenderTarget(GetWorld(),
		RenderTarget, Canvas, Size, Context);
...
void UGKSharedTexture::RemovePoint(FIntPoint Index)
{
	AddPoint(Index, FLinearColor(0, 0, 0, 0));
}

// Called every frame
// void USharedTexture::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
// {
