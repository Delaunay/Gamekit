...
#include "Characters/GKPawnEnchancedInput.h"
...
void AGKPawnEnchancedInput::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AGKPawnEnchancedInput::Tick(float DeltaTime)
{
...
TMap<class UInputAction *, FName> const &AGKPawnEnchancedInput::GetActionNameOverrides() {
    return InputNames;
}

class UInputMappingContext *AGKPawnEnchancedInput::GetDefaultInputMapping() {
...
#include "EnhancedInputSubsystems.h"
 ...
 void AGKPawnEnchancedInput::InitializeEnhancedInput(
    class AController *MyController, class UInputComponent *PlayerInputComponent) {
    // Cache for later
    PlayerEnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent);

    if (PlayerEnhancedInputComponent == nullptr) {
        UE_LOG(LogGamekit, Log,
               TEXT("SetupPlayerInputComponent: Pawn is not using EnhancedInputComponent"));
        return;
    }
...
void AGKPawnEnchancedInput::AddMappingContext(class UInputMappingContext *Input,
                                                    int32                       Priority) {
    if (PlayerEnhancedInputComponent == nullptr) {
        UE_LOG(LogGamekit, Log,
               TEXT("AddMappingContext: Pawn is not using EnhancedInputComponent"));
        return;
    }
    ...
    if (InputSubsystem == nullptr) {
        UE_LOG(LogGamekit, Log,
               TEXT("AddMappingContext: UEnhancedInputLocalPlayerSubsystem is null"));
        return;
    }
...
void AGKPawnEnchancedInput::RemoveMappingContext(class UInputMappingContext *Input) {
    if (PlayerEnhancedInputComponent == nullptr) {
        UE_LOG(LogGamekit, Log,
               TEXT("RemoveMappingContext: Pawn is not using EnhancedInputComponent"));
        return;
    }
    ...
    if (InputSubsystem == nullptr) {
        UE_LOG(LogGamekit, Log,
               TEXT("RemoveMappingContext: UEnhancedInputLocalPlayerSubsystem is null"));
        return;
    }
