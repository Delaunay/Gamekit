...
UGKAsyncTask_GameplayEffectChanged* UGKAsyncTask_GameplayEffectChanged::ListenForGameplayEffectChange(UAbilitySystemComponent * AbilitySystemComponent)
{
	UGKAsyncTask_GameplayEffectChanged* ListenForGameplayEffectStackChange = NewObject<UGKAsyncTask_GameplayEffectChanged>();
	...
	TArray<FActiveGameplayEffectHandle> ActiveHandles = AbilitySystemComponent->GetActiveEffects(FGameplayEffectQuery());
	for (auto ActiveHandle : ActiveHandles) {
																	  ...
																	  FActiveGameplayEffectHandle ActiveHandle)
{
	...
	OnGameplayEffectAdded.Broadcast(ActiveHandle, const_cast<UGameplayEffect*>(EffectDef), Tags, Duration - (CurrentTime - Start));
}
...
void UGKAsyncTask_GameplayEffectChanged::OnGameplayEffectStackChange_Native(FActiveGameplayEffectHandle EffectHandle, int32 NewStackCount, int32 PreviousStackCount)
{
