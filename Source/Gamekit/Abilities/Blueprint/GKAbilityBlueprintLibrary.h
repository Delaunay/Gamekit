...
public:
	...
	FGKGameplayEffectContainerSpec AddTargetsToEffectContainerSpec(
		const FGKGameplayEffectContainerSpec& ContainerSpec,
		const TArray<FHitResult>& HitResults,
		const TArray<AActor*>& TargetActors);

	UFUNCTION(BlueprintCallable, Category = "Ability|Effect")
	TArray<FActiveGameplayEffectHandle> ApplyExternalEffectContainerSpec(const FGKGameplayEffectContainerSpec& ContainerSpec);

	//! Safe way to query the Animation set, should always return a valid animation
	//! NB: the animation can change if multiple animations are available for the same animation kind
