...
struct TMatrix3D
{
    ...
    TMatrix3D(int row, int col, int depth = 1): Row(row), Col(col), Depth(depth) { Data.Init(T(), Row * Col * Depth); }
    ...
    FORCEINLINE void ResetLayer(int d, T Value) { FMemory::Memset(GetLayer(d), Value, GetLayerSizeBytes()); }
    ...
    {
        return FIntVector(FMath::Clamp(P.X, 0, Width() - 1), FMath::Clamp(P.Y, 0, Height() - 1), P.Z);
    }
    ...
    void FillRectangle2D(FIntVector Point1, FIntVector Point2, T Value)
    {
        ...
        FIntVector p1(FMath::Min(Point1.X, Point2.X), FMath::Min(Point1.Y, Point2.Y), 0);

        FIntVector p2(FMath::Max(Point1.X, Point2.X), FMath::Max(Point1.Y, Point2.Y), 0);
    ...
    FORCEINLINE bool Valid(FIntVector V) const { return V.X >= 0 && V.X < Width() && V.Y >= 0 && V.Y < Height(); }
